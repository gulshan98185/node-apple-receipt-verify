...
module.exports = {
  ...
  validate: function (options, cb) {
    ...
    const schema = Joi.object().keys({
      verbose: Joi.boolean().optional(),
      receipt: Joi.string().required(),// removed .base64() for checking validation
    ...
    const result = schema.validate(options,{ allowUnknown: true, convert: true });
    console.warn("SHUBHAM140 Received valid ", result);
    if (result.error) {
    ...
    tasks.push(function (taskCallback) {
      console.warn("SHUBHAM141 Received valid ", options);
